...
export const getOneItemAndUpdate = (req: Request, res: Response) => {
    mongoose
        .model(req.params.collection)
        .findOneAndUpdate({_id: req.params.listId, "listItems._id": req.params.itemId}
            , {$set:{ "listItems.$.name": req.body.name, "listItems.$.quantity": req.body.quantity, "listItems.$.category": req.body.category}}
            , {new: true})
        .then(item => {
            res.json(item);
        })
...
export const addOneItemToList = (req: Request, res: Response) => {
    mongoose
        .model(req.params.collection)
        .findOneAndUpdate({_id: req.params.listId}
...
export const deleteOneItemFromList = (req: Request, res: Response) => {
    mongoose
        .model(req.params.collection)
        .updateOne({_id: req.params.listId}, {$pull: {"listItems": {"_id": req.params.itemId}}})
        .then(item => {
            item.n > 0
                ? res.json(item)
                : res.json("No matches.");
